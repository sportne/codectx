...
def test_parser_version_exits_with_package_version(
    ...
    assert "codectx 1.1.0" in capsys.readouterr().out
...
def test_context_command_generates_markdown_for_file_only_anchor(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    repo = tmp_path / "repo"
    ...
    output = capsys.readouterr().out
    assert "# codectx context bundle" in output
    assert "- anchor_kind: file" in output
    assert "- file: src/Foo.java" in output
    assert "- line_count: 1" in output
